...
<!-- BEGIN: Subheader -->
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h3 class="m-subheader__title ">{{ trans('lang.offers') }}</h3>
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
            ...
            $("#m_table_1").DataTable({
                ...
                serverSide: true,
                order: [[ 9, "desc" ]],
                ajax: '{!! route('provider-offers-data') !!}',
                columns: [
                    { data: 'id' },
                    { data: 'category_id'},
                    { data: 'country',orderable: false, searchable: false},
                    { data: 'location'},
                    { data: 'city_id',orderable: false, searchable: false},
                    { data: 'rate'},
                    { data: 'from'},
                    { data: 'to'},
                    { data: 'end_at'},
            ...
            @if(LaravelLocalization::getCurrentLocale() == 'ar')
                swal({
                    ...
                    function() {
                        $.ajax({
                            type: "GET",
                            url: "{{url("/providers/offers/delete")}}" + "/" + id,
