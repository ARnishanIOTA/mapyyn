			...
			<div class="row">
				<div class="col-md-10 offset-md-1">
					<div class="data-login border p-4 bg-white">
						...
						@endforeach
						{{-- <form action="{{route('buyOffer', ['id' => $offer->id])}}" method="POST" id="forms">
							@csrf
							<button class="btn btn-success" type="submit" >{{ trans('lang.buy') }}</button>
						</form> --}}
						<form action="{{route('buyOffer', ['id' => $offer->id])}}" method="POST" class="paymentWidgets" data-brands="VISA MASTER AMEX">
							@csrf
						</form>
					</div>
				</div>
